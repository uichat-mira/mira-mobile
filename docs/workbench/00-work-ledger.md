...
| MOB-054 | 设置 → 通知 真实功能与真机验收 | REVIEW | PR #152 基于 `dev@0d9f3a0`；设置页「通知」行接通系统通知设置直达（Android 8+ 应用通知设置页 / iOS 应用设置页）+ 失败 Alert 兜底（连按锁定）；副标题恒为中性提示，不伪造授权状态；Typecheck/Lint/Jest 与 Android/iOS 构建由 CI 提供；Android 真机 + iOS 真机验收挂 docs/task-cards/MOB-054-settings-notification-system-entry.md |
