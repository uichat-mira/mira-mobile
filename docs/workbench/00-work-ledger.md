...
| MOB-049 | 个性化设置真机验收 | REVIEW | PR #142 已合入 `dev`；设置项（风格 / 亲和度 / 特征 / 快速回答 / 自定义指令）设备本地持久化（key `mira.mobile.personalization.v1`）已落地；自动化证据（Typecheck/Lint/Jest/双平台 build/AI Review/SonarCloud）在 PR CI 通过；Android 真机 1–9 + iOS 最小集的真机人工验收与结果回写见 `docs/task-cards/MOB-049-personalization-device-acceptance.md`，验收通过前该卡保持 REVIEW |
| MOB-050 | 设置 → 存储 真实功能与真机验收 | REVIEW | 设置页「存储」行历史上是无 actionId 的占位。本卡升级为真实页面 `StorageScreen`，新增纯函数模块 `deviceStorageUsage` 汇总设备本地占用（外观 / 个性化 / 线程本地状态 / 本地 Provider / 拾言草稿与提交 / 拾云端 API / 已提交拾言原始录音），新增 `LocalCaptureRepository.purgeSubmittedAudioFiles` 仅删 `status === 'submitted'` 的本地原始文件不动元数据 / Cloud；提供「清理已提交拾言原始录音」与「清空本地 UI 状态」两个一次性本地维护动作；清空范围限定 UI / 草稿类数据，**保留**本地 Provider 配置与会话、设备安全存储凭据与拾云端 R2（依据 Mira AI Review P2 边界裁定）；`docs/task-cards/MOB-050-storage-screen-and-recovery.md` 列 Android 真机 + iOS 真机验收用例；Typecheck/Lint/Jest + 双平台 CI 已通过；最终真机交互与持久化验收挂 MOB-050 任务卡 |
| MOB-052 | 设置 → 电子邮件 真实功能与真机验收 | REVIEW | 设置页「电子邮件」行历史上是无 actionId 的占位（PR #151 同 MOB-050 / MOB-051 一并记录，本卡为本批最后一例占位升级）。`SettingsRow` 检测到无 actionId 时整行 disabled 并隐藏 chevron，按下无任何响应。本卡升级为真实功能：抽出 `CONTACT_EMAIL` 常量与 `openContactEmail`，通过 `Linking.openURL('mailto:...')` 唤起系统邮件客户端，目标邮箱 `dangjingtao@gmail.com`；无 handler 时弹 Alert 提示用户手动发件，避免「按了按钮没反应」；无新增原生依赖，不动协议 / 凭据 / Host / Provider 边界。`docs/task-cards/MOB-052-email-contact-mailto.md` 列 Android 真机 + iOS 真机验收用例（入口可达、唤起、主题/正文留空、卸载邮件客户端后的兜底、多次连按、外观联动、飞行模式、字段恒定 10 项）。Typecheck/Lint/Jest + 双平台 CI 在 PR #151 跑通；最终真机交互验收挂 MOB-052 任务卡。**路由说明**：用户原始指令目标仓库为 `uichat-mira/uichat-mira-mobile`，本次实际推送仓库为 `uichat-mira/mira-mobile`（当前 token 仅在此仓有 push 权限；两仓 dev 头 SHA 相同），合入 dev 后按 Mira 同步流程把同一 commit SHA 带回 `uichat-mira-mobile` 的 dev |
