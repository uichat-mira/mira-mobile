...
| MOB-050 | 设置 → 存储 真实功能与真机验收 | REVIEW | 设置页「存储」行历史上是无 actionId 的占位。本卡升级为真实页面 `StorageScreen`，新增纯函数模块 `deviceStorageUsage` 汇总设备本地占用（外观 / 个性化 / 线程本地状态 / 本地 Provider / 拾言草稿与提交 / 拾云端 API / 已提交拾言原始录音），新增 `LocalCaptureRepository.purgeSubmittedAudioFiles` 仅删 `status === 'submitted'` 的本地原始文件不动元数据 / Cloud；提供「清理已提交拾言原始录音」与「清空本地 UI 状态」两个一次性本地维护动作，**不**触碰设备安全存储里的配对凭据 / Provider API Key / 拾云端 R2；`docs/task-cards/MOB-050-storage-screen-and-recovery.md` 列 Android 真机 + iOS 真机验收用例；Typecheck/Lint/Jest + 双平台 CI 在 PR 中跑通；最终真机交互与持久化验收挂 MOB-050 任务卡 |
