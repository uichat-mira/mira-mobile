...
| 10 | 字段恒定（源码范围） | 在 `src/` 实现源码内 grep `dangjingtao@gmail.com`，并全仓 grep 作对照 | `src/` 内仅命中两处：`CONTACT_EMAIL` 常量定义、行副标题对 `CONTACT_EMAIL` 的模板引用；全仓额外命中只允许出现在任务卡 / 台账等协作文档中，不允许出现在其它实现脚本、配置或 PR 注释里 |
